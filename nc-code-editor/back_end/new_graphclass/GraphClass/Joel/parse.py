...
def parseData(csv):
    ...
    if df.isna().values.any():
        raise ValueError(
            "Error: No columns are allowed to be empty, no leading whitespace is allowed"
        )
    ...
    for i in range(len(keys)):
        ...
        for x in range(len(currkey)):
            ...
            if (
                ...
                raise ValueError(
                    "Error: All relationships must have corresponding value"
                )
            ...
            if key.title() == DIRECTED:
                ...
                if offset == 1:
                    ...
                    if len(rel) != 2:
                        raise ValueError(
                            "Error: Incorrect directed relationship format"
                        )
